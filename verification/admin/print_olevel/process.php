...
if($_SESSION['olv_veri_who'] == 'pros'){
    ...
    $olevel_submissionSQL = sprintf("SELECT v.*,c.coltitle, p.fname, p.lname, p.mname, COUNT(ovd.stdid) AS sitting "
                                    ...
                                    . "WHERE ovd.sesid = %s AND ovd.treated = 'No' "
...
do{
    ...
    $olevel_submission_dataSQL = sprintf("SELECT ovd.*, c.colid "
                                        ...
                                        . "WHERE ovd.stdid = %s AND ovd.treated = 'No' "
                                        . "AND ovd.approve = 'Submitted'", 
                                         $query_part, GetSQLValueString($olevel_submission['stdid'], 'text'));
                            ...
                            <div class="box box-bordered box-color">
                                <div class="box-title">
                                    <h3><i class="icon-reorder"></i>
                                        <?= ($_SESSION['olv_veri_who'] == 'pros')? 'Prospective ' : 'Returning '?> Student O'Level Verification Code Generation Page (<?= $row_col['coltitle'] ?>)
                                    </h3>
                                    <ul class="tabs">
                                        <li class="active">
