...
if(isset($_POST['MM_Search']) && $_POST['MM_Search'] == 'form1'){
    ...
    switch ($_POST['utype']) {
        case 'pros':
            $title = "O'Level Verification Report for {$row_session['sesname']} Prospective Student ";
            
            $query = sprintf("SELECT olv.stdid, olv.exam_year, olv.exam_type, olv.exam_no, p.fname, p.lname, pr.progname "
                            . "FROM olevel_veri_data olv, prospective p, programme pr "
                            . "WHERE olv.stdid = p.jambregid "
                            . "AND olv.progid = pr.progid "
                            . "%s "
                            . "%s "
                            . "%s ORDER BY p.jambregid, olv.progid ASC ", $level, $prog, $ses);
        ...
        case 'stud':
            ...
            $query = sprintf("SELECT olv.stdid, olv.exam_year, olv.exam_type, olv.exam_no, s.fname, s.lname, pr.progname "
                            ...
                            . "%s "
                            . "%s "
                            . "%s ORDER BY s.stdid, olv.progid ASC ", $level, $prog, $ses);
