...
$query_prog = sprintf("SELECT progid, progname "
                    . "FROM programme "
                    . "WHERE continued = 'Yes' ORDER BY progid DESC");
...
$query = sprintf("SELECT v.*, p.jambregid, p.pstdid, p.fname, 
...
if (isset($_POST['MM_Submit']) && $_POST['MM_Submit'] == 'change_prog') {
    ...
    if ($_POST['action'] == 'yes') {
        ...
        $queryClear = sprintf("DELETE FROM prog_options WHERE jambregid = %s ",
        ...
        $msg = "Congratulations! Your O`level result has been verified "
                . "But it does NOT meet the requirement of the programme "
                . "you applied for.However below are other programmes that you can choose one from "
                ...
                . "below text box and click verified so that you"
        ...
        $query = sprintf("UPDATE verification "
                        . "SET  status = 'change_prog', msg = %s, "
                        . "released_by = %s, date_treated = CURDATE(), date_released = CURDATE() "
                        . "WHERE id = %s ", 
...
//                . " verified and it meet the requirement of the"
                            ...
                            <div class="box box-bordered box-color">
                                <div class="box-title">
                                    <h3><i class="icon-reorder"></i>
                                        Prospective Student O'Level Verification 
                                        Code Generation Page (<?= $row_college['coltitle'] ?>)
                                    </h3>
                                    <ul class="tabs">
                                        <li class="active">
                                            <a href="my_treated.php" target="_new">My Treated</a>
                                        </li>
                                    </ul>
                                </div>
...
<!--                                                            <a  ng-click="refere($index, cd)" class="btn btn-small btn-blue">Contact Adm. Officer</a>-->
                                                        </td>  
                                                    </tr>
                                                </tbody>
                                            </table> 
