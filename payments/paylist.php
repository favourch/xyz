...
if(isset($_GET['pid'])) {
    ...
    if (getAccess() == 2) {
        $filter .= ' AND d.colid = ' . GetSQLValueString($colname_col, 'int');
    } elseif (getAccess() == 3) {
        $filter .= ' AND d.deptid = ' . GetSQLValueString($colname_dept, 'int');
    }
}
...
$query_stud = sprintf("SELECT distinct(s.stdid), s.fname, s.lname, at.typename, l.levelname, s.sex, st.date_time "
                    ...
                    . "JOIN level_name l ON l.levelid = ps.level AND l.active = 'TRUE' "
                    . "WHERE ps.sesid = %s "
