...
function getCgpa($studid, $progid, $ses, $sem, $tams, &$examined, $courses, $courses1, $type, $grad) {
    ...
    $query_cgpa = sprintf("SELECT distinct cr.csid, c.semester, r.tscore, r.escore, r.tscore+ r.escore as score, cr.sesid, "
            . "dc.status, dc.unit, c.status as cstatus, c.unit as cunit, t.accepted, t.upload "
            . "FROM course_reg cr "
    ...
    for($i = 0; $i < $totalRows_cgpa; $i++, $row_cgpa = mysql_fetch_assoc($cgpa)) {
        ...
        if($row_cgpa['upload'] == 'yes' && $row_cgpa['accepted'] == 'yes') {
            if(!is_null($row_cgpa['tscore']) || !is_null($row_cgpa['escore'])) {
                $nr = false;
            }             
            if (!$nr) {
            if(is_null($row_cgpa['tscore']) || !is_numeric($row_cgpa['tscore'])) {
                $tf = true;
            }    
        }
        }
        ...
        if(!$tf && $passmark <= $row_cgpa['score']) {            
            ...
            unset($courses[$key]);
        }else if($status != 'Elective' && !in_array($row_cgpa['csid'], $courses) && ($nr || $tf || $passmark > $row_cgpa['score'])) {
        ...
        if($sesn == $ses) {
            ...
            if(strtolower($row_cgpa['semester']) == strtolower($sem)) {
                ...
                $crs[$row_cgpa['csid']] = [$row_cgpa['score'], $tf, $nr];
            }else{
                $gp = gradepoint($unit, $row_cgpa['score'], $sesn, $grad, $tf);
                $prevValue += $gp;
                $prevUnit +=  $unit;
...
$html = '<p>PROGRAMME: '.$row_info['progname'].'</p>
    ...
    <p>NO REC FOR COMMENDATION: '.$recommended.'</p>
    <p>NO REC FOR PROBATION: '.$probation.'</p>
    <p>NO REC FOR WITHDRAWAL:  '.$withdraw.'</p>
    <p>NO ON WARNING: '.$withdraw.'</p><br/>
    <p>HOD\'S NAME AND SIGNATURE: '.$hod_name.'    _____________________________</p>
    <p>DEAN\'S NAME AND SIGNATURE: '.$dean_name.'    _____________________________</p>
    <p>CHAIRMAN VETTING COMMITTEE\'S NAME:  _____________________________ REMARK:_______________ SIGNATURE:____________________</p>
