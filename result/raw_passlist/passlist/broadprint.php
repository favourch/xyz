...
$query_info = sprintf("SELECT colname, c.colid, deptname, d.deptid, progname, degree "
...
$hod_name = sprintf("%s %s%s %s",
        $row_hod['title'], 
        substr($row_hod['fname'], 0, 1),
        (isset($row_hod['mname']))? '.'.substr($row_hod['mname'], 0, 1): '',
        $row_hod['lname']);
...
$dean_name = sprintf("%s %s%s %s",
        $row_dean['title'], 
        substr($row_dean['fname'], 0, 1),
        (isset($row_dean['mname']))? '.'.substr($row_dean['mname'], 0, 1): '',
        $row_dean['lname']);

$query_vc = sprintf("SELECT title, lname, fname "
                    . "FROM lecturer l, appointment a "
                    . "WHERE l.lectid = a.lectid "
                    . "AND postid=1",
                    GetSQLValueString($row_info['colid'], "int"));
$vc = mysql_query($query_vc, $tams) or die(mysql_error());
$row_vc = mysql_fetch_assoc($vc);
$totalRows_vc = mysql_num_rows($vc);
$vc_name = sprintf("%s %s%s %s",
        $row_vc['title'], 
        substr($row_vc['fname'], 0, 1),
        (isset($row_vc['mname']))? '.'.substr($row_vc['mname'], 0, 1): '',
        $row_vc['lname']);

$query_list = sprintf("SELECT *, s.stdid as stdid, r.stdid as rstdid "
                    . "FROM student s "
                    . "LEFT JOIN registration r ON s.stdid = r.stdid "
                    . "JOIN admission_type at ON s.admid = at.typeid "
                    . "WHERE s.progid=%s "
                    . "AND r.sesid = %s "
                    . "AND r.level = %s AND s.passlist = 'no' "
                    . "AND SUBSTR(s.stdid,1,4) > 2012 "             //take away extra year students
...
$outstanding = []; // hold other courses
$outstanding1 = []; // hold 100 level courses.
...
for($i = 0; $i < $totalRows_list; $i++, $row_list = mysql_fetch_assoc($list)) {
    if(!isset($row_list['rstdid']) || $row_list['rstdid'] == '') {
        $absent++;
        continue;
    }
    ...
    if($cgpa['cum']['gpa'] < 1.0 && ($level != 1 && $semester != 'F')) {
        $probation++;
    }
...
function grade($score, $level, $tf){
    $value = '';
    global $sesGrad;
    global $session;

    $grades = $sesGrad[$session][$level];
...
function getCgpa($studid, $progid, $ses, $sem, $tams, &$examined, $courses, $courses1, $type, $grad) {
    ...
    $query_cgpa = sprintf("SELECT distinct cr.csid, c.semester, r.tscore, r.escore, r.tscore+ r.escore as score, cr.sesid, "
            ...
            . "FROM course_reg_passlist cr "
            . "LEFT JOIN result_passlist r ON cr.stdid = r.stdid AND cr.sesid = r.sesid AND cr.csid = r.csid "
    ...
    for($i = 0; $i < $totalRows_cgpa; $i++, $row_cgpa = mysql_fetch_assoc($cgpa)) {
        ...
        if(!$tf && $passmark <= $row_cgpa['score'] || in_array($status, ['Elective', 'Required'])) {            
            ...
            unset($courses[$key]);
        }else if(!in_array($status, ['Elective', 'Required']) && !in_array($row_cgpa['csid'], $courses) && ($tf || $passmark > $row_cgpa['score'])) {
        ...
        if($sesn == $ses) {
            ...
            if(strtolower($row_cgpa['semester']) == strtolower($sem)) {
                ...
                $crs[$row_cgpa['csid']] = [$row_cgpa['score'], $crs_level, $tf];
            }else{
                $gp = gradepoint($unit, $row_cgpa['score'], $crs_level, $sesn, $grad, $tf);
                $prevValue += $gp;
                $prevUnit +=  $unit;
...
<div style="text-align:center; width:100%; font-size: 16pt">
    <div style="float:left;text-align:center; width:60%; font-size: 16pt">SUMMARY OF RESULTS</div>
...
$html = '<p>PROGRAMME: '.$row_info['degree'].' '.$row_info['progname'].'</p>
    <p>DEPARTMENT: '.$row_info['deptname'].'</p>
    <p>SESSION:         '.$row_rssess['sesname'].'</p>
    <p>SEMESTER:       '.ucwords($sem).'</p>
    <p>LEVEL:         '.$level.'00L</p><br/>
    <p>NO OF STUDENTS: '.$totalRows_list.'</p>
    <p>NO ABSENT: '.$absent.'</p>
    <p>NO WITH CLEAR PASSES: '.$passed.'</p>
    <p>NUMBER WITH REFERENCES: '.$failed.'</p>
    <p>NO REC FOR COMMENDATION: '.$recommended.'</p><br/><br/>
    <p>HOD\'S NAME AND SIGNATURE: '.$hod_name.'    _____________________________</p><br/>
    <p>DEAN\'S NAME AND SIGNATURE: '.$dean_name.'    _____________________________</p><br/>
    <p>CHAIRMAN VETTING COMMITTEE\'S NAME:  _____________________________ REMARK:_______________ SIGNATURE:____________________</p><br/>
    <p>VICE CHANCELLOR\'S NAME AND SIGNATURE: '.$vc_name.' ______________________________</p>
    <br/>';
    
    /* removed from above html
     <p>NO REC FOR PROBATION: '.$probation.'</p>
    <p>NO REC FOR WITHDRAWAL:  '.$withdraw.'</p>
    <p>NO ON WARNING: '.$withdraw.'</p><br/>
    */
...
if(!empty($students)) {
    ...
    foreach($students as $count => $std) {
        ...
        foreach($course as $c) {
            ...
            if(isset($std['crs'][$c]) && $std['crs'][$c] != '') {
                $html .= "<td align='center'>{$std['crs'][$c][0]}<br/>".grade($std['crs'][$c][0], $std['crs'][$c][1], $std['crs'][$c][2])."</td>";
                continue;
            }
    ...
    $html .= "<table width='100%' border='0'>
      <tr>
        <td width='{$width}' align='center'>{$hod_name}</td>
        <td width='{$width}' align='center'>{$dean_name}</td>";
        
        if($final) {
            $html .= "<td width='{$width}' align='center'>{$vc_name}</td>";
        }
