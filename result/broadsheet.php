...
if(getAccess() == 2) {
    $query_prog = sprintf("SELECT progid, progname, duration "
                            ...
                            . "WHERE d.deptid = p.deptid AND p.continued = 'Yes'"
                            . "AND d.colid=%s "
...
$query_level = "SELECT * FROM level_name WHERE active='TRUE' ";
$level = mysql_query($query_level, $tams) or die(mysql_error());
$totalRows_level = mysql_num_rows($level);

$query_curriculum = "SELECT * FROM curriculum WHERE status = 'active' ";
...
$query_Rssess = "SELECT * FROM `session` WHERE listing = 'TRUE' ORDER BY sesname DESC";
...
<!--                                                        <option value="<?php echo $duration + 1 ?>">Extra Year 1</option>
                                            ...
                                            <div class="row-fluid">
                                                <div class="span3">Choose Curriculum: </div>
                                                <div class="span4">  
                                                    <select name="curriculum" id="curriculum" onchange="sesFilter(this)">
                                                        <option value="-1">--Curriculum--</option>
                                                        <?php for (; $row_curriculum = mysql_fetch_assoc($curriculum);) : ?>
                                                            <option value="<?php echo $row_curriculum['curid'] ?>">
                                                                <?php echo $row_curriculum['curname'] ?>
                                                            </option>
                                                        <?php endfor; ?>
                                                        </option>
                                            ...
                                            <input type="submit" name="submit" id="submit" value="Process Broadsheet" 
