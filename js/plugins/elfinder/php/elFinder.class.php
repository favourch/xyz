...
class elFinder {
	...
	protected $commands = [
		...
		'upload'    => ['target' => true, 'FILES' => true, 'mimes' => false, 'html' => false, 'upload' => false, 'name' => false],
		'get'       => ['target' => true],
		...
		'pixlr'     => ['target' => false, 'node' => false, 'image' => false, 'type' => false, 'title' => false]
		];
	 ...
	 **/
	 protected function pixlr($args) {
	 	...
	 	if (! empty($args['target'])) {
		 	$args['upload'] = [ $args['image'] ];
			$args['name']   = [ preg_replace('/\.[a-z]{1,4}$/i', '', $args['title']).'.'.$args['type'] ];
