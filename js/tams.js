...
function doFilter( action, elem ){
	...
	if( action == "dept"){
		...
		if( params['curid'] )
			location += "&curid="+params['curid'];
		window.location = location;
		
	}else if(action == "curid"){
	    var location = "?filter="+action+"&curid="+selectedOption.value;
		if( params['catid'] )
...
function batchfilt( elem ){
	filt( "batch", elem)
}

function admfilt( elem ){
	filt( "adm", elem)
}
...
function filt( action, elem ){
	...
	if( action == "batch"){
            var loc = "?bid="+selectedOption.value;
            if(params['sid'] )
                loc += "&sid="+params['sid'];
            if(params['aid'] )
                loc += "&aid="+params['aid'];
    ...
    if( action == "adm"){
            var loc = "?aid="+selectedOption.value;
            if(params['sid'] )
                loc += "&sid="+params['sid'];
            if(params['bid'] )
                loc += "&bid="+params['bid'];
	...
	if( action == "cur"){
		var loc = "?cur="+selectedOption.value;
		if( params['pid'] )
			loc += "&pid="+params['pid'];
		if( params['did'] )
			loc += "&did="+params['did'];
	...
	if( action == "loc"){
		var loc = "?loc="+selectedOption.value;
		if(params['sid'] )
			loc += "&sid="+params['sid'];
		if(params['curid'] )
			loc += "&curid="+params['curid'];	
		if(params['ssid'] )
			loc += "&ssid="+params['ssid'];
                if(params['pid'] )
                    loc += "&pid="+params['pid'];
	...
	if( action == "lg"){
		loc = "?filter="+action+"&lg="+selectedOption.value;
		if( params['schcat'] )
			loc += "&cat="+params['schcat'];
		window.location = loc;
	}
}
