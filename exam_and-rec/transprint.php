...
for($idx =0; $idx < $totalRows_grad; $idx++, $row_grad = mysql_fetch_assoc($grad)) {
 ...
 $query_result = sprintf("SELECT distinct (r.csid), r.sesid, c.csname, c.semester, r.tscore+ r.escore as score, s.sesname, dc.status, dc.unit, c.status as cstatus, c.unit as cunit, dc.level "
        . "FROM `result_passlist` r, department_course dc, course c, session s, teaching t "
        . "WHERE stdid = %s "
        . "AND c.csid = r.csid "
        ...
        . "FROM `result_passlist` r, course c, session s, teaching t "
        . "WHERE stdid = %s "
        . "AND c.csid = r.csid "
        . "AND r.csid = t.csid "
        . "AND s.sesid = t.sesid "
...
<h2 style="font-size: 30pt">'.$university.'</h2>
<h3 style="font-size: 25pt">'.$row_stud['colname'].'<br></h3>
<h4 style="font-size: 17pt">'.$row_stud['deptname'].'</h4>
</td>
</tr>
</table>
<div style="text-align:center; width:100%; font-size: 16pt">Result Profile</div>';
...
$html .= '<table width="15%" class="table table-bordered" style="font-size: 20pt">
            <tr>
                <td>';
$html .= '
            ...
            CGPA: ' . $totalStats['cgpa'] . '  CLASS OF DEGREE:  '. getCgrade($totalStats['cgpa']) .'<br/>
        ';
$html .= '</td>
            </tr>'
        . '</table>';
