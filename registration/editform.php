...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form2")) {    
    ...
    if(isset($_POST['updated_entries']) && !empty($_POST['updated_entries'])) {
        ...
        $deleteList = array_diff($updatedCodes, $deletedEntries);        
        $delete = implode("','", $deleteList);
    ...
    if(isset($_POST['approved_entries']) && !empty($_POST['approved_entries'])) {
        ...
        $updateSQL = sprintf("UPDATE course_reg SET cleared = 'TRUE' "
                                . "WHERE stdid = %s "
                                . "AND sesid = %s "
                                . "AND csid IN ('%s')",
                               GetSQLValueString($_POST['stid'], "text"),
    ...
    if(isset($_POST['courses']) && !empty($_POST['courses'])) {
        ...
        if($totalRows_courChk > 0) {
            for($idx = 0; $idx < $totalRows_courChk; $idx++, $row_courChk = mysql_fetch_assoc($courChk)) {            
                $excludeList[] = $row_courChk['csid'];
            }        
        }

        $includeList = array_diff($uniqueCourses, $excludeList);
        foreach ($includeList as $l) {
            $dbEntry = sprintf("(%s, %s, %s, %s)", 
                    GetSQLValueString($_POST['stid'], "text"), 
                    GetSQLValueString(htmlentities($l), "text"), 
                    GetSQLValueString($_POST['sid'], "int"), 
        ...
        $insertSQL = sprintf("INSERT INTO course_reg (stdid, csid, sesid, cleared) VALUES %s;",
...
$query_sess = sprintf("SELECT * FROM `session` WHERE sesid = %s", GetSQLValueString($sesid, "text"));
...
$query_suggestion = sprintf("SELECT csid, status, csname, unit "
                        . "FROM course c "
                        . "WHERE (deptid = %s "
                        . "OR catid IN (3,4,5,8)) AND c.curid = %s "
                        . "AND level IN (%s) "
...
for($idx = 0 ; $totalRows_suggestion > $idx; $idx++, $row_suggestion = mysql_fetch_assoc($suggestion)) {
 ...
 $query_registered = sprintf("SELECT distinct(r.csid), c.semester, c.csname, dc.status, dc.unit "
...
$name = (isset($row_stud['stdid'])) ? "for ".$row_stud['lname']." "
        .$row_stud['fname']." - ".$sesname." Session": "";
?>
<!doctype html>
<html ng-app="tams">
...
<?php include INCPATH."/page_header.php" ?>
                                ...
                                <div class="box-content">
                                    <div class="row-fluid">
                                        <form action="<?php echo $editFormAction; ?>" ng-submit="submitAction($event)" 
                                            ...
                                            <input name="sid" type="hidden" value="<?php echo $sesid ?>" />
    ...
    module.controller('CourseController', function($scope, $timeout, $interpolate) {
        ...
        var courses = new Bloodhound({
            ...
            remote: 'suggestions.php?l=<?php echo $row_stud['level']; ?>&stid=<?php echo $colname_stud ?>&q=%QUERY',
