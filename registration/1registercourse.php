...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
    ...
    $insertSQL = sprintf("INSERT INTO registration (stdid, sesid, status, course) VALUES (%s, %s, %s, %s)", GetSQLValueString($_POST['stid'], "text"), GetSQLValueString($_POST['sid'], "int"), GetSQLValueString("Registered", "text"), GetSQLValueString("Unregistered", "text"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form2")) {
    ...
    if (isset($_POST['courses']) && !empty($_POST['courses'])) {
        ...
        $insertSQL = sprintf("INSERT INTO result (stdid, csid, sesid, cleared) VALUES %s;", GetSQLValueString($finalCourses, "defined", $finalCourses));
...
$query_course = sprintf("SELECT r.csid, c.semester, c.csname, status, unit "
        ...
        . "AND c.csid = r.csid "
        . "AND r.sesid = %s "
        . "ORDER BY r.csid, c.semester ASC", GetSQLValueString($colname_regStatus, "text"), GetSQLValueString($colname_regsess, "int"));
$course = mysql_query($query_course, $tams) or die(mysql_error());
$row_course = mysql_fetch_assoc($course);
$totalRows_course = mysql_num_rows($course);

$query_crslist = sprintf("SELECT csid FROM result WHERE stdid = %s AND sesid = %s", GetSQLValueString($colname_regStatus, "text"), GetSQLValueString($colname_regsess, "int"));
...
$query_studpop = sprintf("SELECT * FROM registration WHERE stdid = %s AND sesid = %s", GetSQLValueString($colname_regStatus, "text"), GetSQLValueString($colname_regsess, "int"));
...
$query_suggestion = sprintf("SELECT csid, status, csname, unit "
        ...
        . "FROM result "
        . "WHERE stdid = %s "
        . "AND sesid = %s)", GetSQLValueString(getSessionValue('did'), "int"), GetSQLValueString(" ", "defined", implode(',', $levels)), GetSQLValueString($colname_stud, "text"), GetSQLValueString($row_sess['sesid'], "int"));
...
$query_registered = sprintf("SELECT r.csid, status, csname, unit "
        ...
        . "WHERE c.csid = r.csid "
        . "AND stdid = %s "
        . "AND sesid = %s", GetSQLValueString($colname_stud, "text"), GetSQLValueString($row_sess['sesid'], "int"));
