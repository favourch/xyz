...
if(!isset($_SESSION['payment']['gateway'])) {
    ...
    if (strpos($referer, 'fee_payment/mastercard/index') == false) {   
    ...
    if (isset($_POST['paynow']) && isset($_SESSION['payment'])) {
        ...
        if (!(curl_errno($ch) > 0)) {
            ...
            $sql = sprintf("INSERT INTO schfee_transactions "
                    . "(can_no, can_name, amt, sesid, scheduleid, reference, year, status, date_time, ordid, sessionid, "
                    ...
                    GetSQLValueString($scheduleid, "text"),
...
<a href="processpayment.php">
    <button class="btn btn-primary">Try This Payment Again</button>
</a>