...
$query_history = sprintf('SELECT p.fname, p.lname, p.mname, p.stdid, at.can_name, at.ordid, '
                        . 'at.status, at.reference, at.amt, at.date_time '
                        . "FROM tp_transactions at "
                        . "JOIN student p ON p.stdid = at.matric_no "
...
if($totalRows_history > 0) {
    $html ='<p style="text-align:center; font-size: 20pt; margin-bottom: 30px"><strong>'.$row_history['typename'].' ICT PROCESSING FEES</strong></p>
    <div style="text-align:center; width:100%; font-size: 20pt">
        <table align="center" style="width: 60%;" class="table table-bordered table-striped">
            ...
            <tr>
                <td>UTME Reg No :</td>
                <td>'.$row_history['stdid'].'</td>
            </tr>
            <tr>
                <td>Description:</td>
                <td>ICT PROCESSING FEES</td>
            </tr>
...
$mpdf->WriteHTML($html);
$mpdf->Output('TP_Repost_receipt.pdf', 'I');
