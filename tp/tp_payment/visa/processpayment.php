...
$purpose = "ICT PROCESSING FEE";

$description = $revhead . "^ICT PROCESSING FEES/" . $sesid . "/" . $canNo . "^" . $purpose . "^";

$path = SITEURL."/tp/tp_payment";
...
if (!(curl_errno($ch) > 0)) {
    ...
    $amt = 'NGN'.number_format($price/100);
    
    $sql = "INSERT INTO "
            . "tp_transactions (matric_no, can_name, sesid, level, reference, amt, year, status, "
            . "date_time, ordid, sessionid, gatewayurl, percentPaid) "
            . "VALUES('$canNo', '$canName', '$sesid', '$level', '$ref', '$amt', '$year', '$status', "
