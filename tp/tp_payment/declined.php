...
//echo getSessionValue('uid'); die();

$query = sprintf("SELECT * FROM student WHERE stdid=%s", 
...
if (isset($_POST["xmlmsg"])) {
    ...
    $update = sprintf("UPDATE  tp_transactions SET resp_code='" . $rc . "',resp_desc = '" . $res . "', auth_code='" . $ac . "', pan='" . $pan . "', status = 'DECLINED', name='" . $name . "', date_time='" . $dt . "', xml = '" . $xmlmsg . "' WHERE  ordid='" . $ordid . "'"); 
...
switch ($rc) {
 ...
 $query_paid= sprintf("SELECT * FROM tp_transactions WHERE ordid=%s",
...
if ($row_result['email'] != null) {
    ...
    $to = $row_result['email'];
    $subject = "{$school_short_name} Portal:  DECLINED ICT PROCESSING FEE PAYMENT";
    $from = "noreply@{$domain}";
