...
if (isset($_POST['paynow']) && isset($_POST['stdid']) && $_POST['form_trig'] == 'form1') {
    ...
    $purpose = "FINAL YEAR CLEARANCE FEE";

    $description = $revhead . "^final year clearance/" . $sesid . "/" . $canNo . "^" . $purpose . "^";

    $path = SITEURL."/clearance_fee";
    ...
    if (!(curl_errno($ch) > 0)) {
        ...
        $sql = "INSERT INTO clearance_transactions ( "
                . "matric_no,can_name, reference, year, status, sesid, "
                . "date_time, ordid, sessionid, gatewayurl, "
                . "percentPaid) "
                . "VALUES('$canNo','$canName','$ref','$year' ,'$status' ,'$sesid','$date','$orderid','$sessionid','$gateway_url','$percent')";
