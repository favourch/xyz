...
$researchSQL = sprintf("SELECT * "
                    . "FROM research r "
                    . "JOIN pub_type p ON p.id = r.pub_type_id "
                    . "JOIN research_area ra ON ra.id = r.res_area_id "
                    . "WHERE lectid = %s ",
...
while ($research = mysql_fetch_assoc($researchRS)) {
    ...
    $data = array(
        ...
        'res_area' => $research['area'],
        'pub_type' => $research['pub_type'],
        'pub_type_id' => $research['pub_type_id'],
        'res_area_id' => $research['res_area_id'],
...
$res_areaSQL = sprintf("SELECT * "
                    . "FROM research_area ");
$researchRS = mysql_query($res_areaSQL, $tams) or die(mysql_error());

$res_areas = array(); 
while($row_res_area = mysql_fetch_assoc($researchRS)){
   
    array_push($res_areas, $row_res_area);
}
$res_areas = json_encode($res_areas);


//Get All Research areas
$pub_typeSQL = sprintf("SELECT * "
                    . "FROM pub_type ");
$pubtypehRS = mysql_query($pub_typeSQL, $tams) or die(mysql_error());

$pub_types = array(); 
while($row_pub_type = mysql_fetch_assoc($pubtypehRS)){
   
    array_push($pub_types, $row_pub_type);
}
$pub_types = json_encode($pub_types);
...
if (isset($_POST['MM_Submit']) && $_POST['MM_Submit'] == 'form1') {
    ...
    $insertResSQL = sprintf("INSERT "
            ...
            . "pub_year, res_area_id, pub_type_id, pub_at, volume, page_num, reference ) "
    ...
    if ($affected) {
        ...
        $notification->set_notification('Operation Successful. Kindly refresh the page ONCE to view the new Publication that is just added', 'success');
    } else {
...
<html ng-app="research">
    ...
    <script>
        ...
        var res_areas = <?= $res_areas ?>;
        var pub_type = <?= $pub_types?>;
    </script>
                                                        ...
                                                        <table class="table table-bordered table-condensed table-striped">
                                                            ...
                                                            <tbody>
                                                                <tr ng-repeat="r in research">
                                                                    ...
                                                                    <td>
                                                                        <a href="#abstract" data-toggle="modal" ng-click="setSelected(r)" title="View Abstract"><i class="glyphicon-notes_2"></i></a>
