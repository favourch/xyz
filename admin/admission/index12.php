...
$query_admit = sprintf("SELECT progoffered, count(pstdid) as count "
        ...
        . "GROUP BY progoffered", GetSQLValueString($sid, "int"), GetSQLValueString($filter, "defined", $filter));
...
// changed to reflect UTME paid/form submitted Applicants::first choice
 $query_first = sprintf("SELECT progid1, at.status, count(p.pstdid) as count "
        ...
        . "AND p.formsubmit = 'Yes' %s "
        . "GROUP BY progid1 "
        . "HAVING progid1 IS NOT NULL", GetSQLValueString($sid, "int"), GetSQLValueString($filter, "defined", $filter)); 
...
// changed to reflect DE paid/form submitted Applicants::second choice
...
$query_admtype = sprintf("SELECT admid, typename "
        . "FROM admissions a "
        . "JOIN admission_type at ON a.typeid = at.typeid "
        . "WHERE a.sesid = %s", 
        GetSQLValueString($sid, text));
