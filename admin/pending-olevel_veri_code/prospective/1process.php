...
$query = sprintf("SELECT DISTINCT (v.stdid), v.sesid, v.ver_code, v.verified, v.release_code, v.refer, v.msg, v.released_by, v.date_treated, olvd.date_treated AS date_printed, p.jambregid, p.pstdid, p.fname, p.lname,(SELECT  count(DISTINCT (exam_no)) FROM olevel_veri_data i  WHERE i.stdid = p.jambregid AND i.approve = 'Yes') AS sittings , p.mname ,p.progoffered, pg.progname "
                . "FROM `verification` v "
                ...
                . "AND v.verified <> 'TRUE' "
                . "AND v.release_code <> 'yes' AND v.refer = 'FALSE' ) AND v.stdid IN (SELECT  DISTINCT (i.stdid) FROM olevel_veri_data i  WHERE i.stdid = p.jambregid AND i.approve = 'Yes') ORDER BY olvd.date_treated ", 
                $query_part,
...
while ($verify_row = mysql_fetch_assoc($verify)) {
    ...
    $code[] = array(
            ...
            'date_printed'  => $verify_row['date_printed']
...
if (isset($_POST['submit'])) {
    ...
    mysql_query("BEGIN");
        ...
        $query = sprintf("UPDATE verification "
                        . "SET release_code = 'yes', msg= %s, released_by = %s, date_treated = CURDATE() "
                        . "WHERE id = %s ",
...
if (isset($_POST['refer'])) {
    ...
    mysql_query("BEGIN");
        ...
        $msg = " Your O`level result has been verified But "
                . "does not meet the requirement of  the programme applied for "
                . " <br/>(<strong>{$prog_choice}</strong>)<br/>."
                . "Howerver,  Kindly contact the admissions office for change of course";
                                                ...
                                                <table class="table  table-condensed table-striped table-hover">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th width="3%">Sittings</th>
                                                            <th width="20%">Programme Choice</th>
