...
$query = sprintf("SELECT v.*, p.jambregid, p.pstdid, p.fname, p.lname,(SELECT  count(DISTINCT exam_no) FROM olevel_veri_data i  WHERE i.stdid = p.jambregid AND approve = 'yes') AS sittings , p.mname ,p.progid1, pg.progname "
                . "FROM `verification` v  JOIN olevel_veri_data olvd ON olvd.stdid = v.stdid "
                ...
                . "AND v.verified <> 'TRUE' "
                . "AND v.release_code <> 'yes' AND refer = 'FALSE' ) ", 
                $query_part,
...
if (isset($_POST['submit'])) {
    ...
    mysql_query("BEGIN");
        ...
        $query = sprintf("UPDATE verification "
                        . "SET release_code = 'yes', treated = 'yes', msg= %s, released_by = %s, date_treated = CURDATE() "
                        . "WHERE id = %s ",
...
if (isset($_POST['refer'])) {
    ...
    mysql_query("BEGIN");
        ...
        $msg = " Your O`level result has been verified But "
                ...
                . "Howerver,  Kindly contact the admission office for change of course";
