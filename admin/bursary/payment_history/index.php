...
if(isset($_POST['search']) && $_POST['search'] != ""){
    ...
    if($num_row_search < 1){
        $query = sprintf("SELECT s.jambregid AS stdid, s.jambregid, s.fname, s.lname, '-' AS level, s.mname, p.progname "
                . "FROM prospective s LEFT JOIN programme p ON p.progid = s.progid1 "
                . "WHERE pstdid LIKE %s "
                . "OR jambregid LIKE %s "
                . "OR fname LIKE %s "
                . "OR lname LIKE %s ", GetSQLValueString(trim($_POST['search']), 'text'), GetSQLValueString(trim($_POST['search']), 'text'), GetSQLValueString(trim($_POST['search']), 'text'), GetSQLValueString(trim($_POST['search']), 'text'));
        $Rs_search = mysql_query($query) or die(mysql_error());
        $row_search = mysql_fetch_assoc($Rs_search);
        $num_row_search = mysql_num_rows($Rs_search);
        $utype = "&utype=pros";
                                                ...
                                                <tbody>
                                                    <?php if( $num_row_search > 0){?>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td><a target="_tab" href="history.php?stdid=<?=$row_search['stdid'].$utype?>" class="btn btn-small btn-blue">Pay. History</a></td>
                                                        </tr>
