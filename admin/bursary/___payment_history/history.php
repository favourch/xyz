...
$query_app_fee = sprintf("SELECT a.*, s.sesname "
        ...
        . "AND a.status = 'APPROVED' ", GetSQLValueString($row_search['jambregid'], 'text'), GetSQLValueString($row_search['stdid'], 'text'));
...
$query_sch_fee = sprintf("SELECT a.*, s.sesname "
        ...
        . "OR matric_no = %s "
        . "OR matric_no = %s"
        . "AND a.status = 'APPROVED' ", GetSQLValueString($row_search['jambregid'], 'text'), GetSQLValueString($row_search['stdid'], 'text'), GetSQLValueString($row_search['jambregid'], 'text'), GetSQLValueString($row_search['stdid'], 'text'));
...
//Create connection to graduands database
$con_username = "tasueded_grad";
$con_password = "123gradportal456";
$grad_DB_name = "tasueded_gradportal";

$grand = mysql_pconnect('localhost', $con_username, $con_password) or trigger_error(mysql_error(), E_USER_ERROR);
mysql_select_db($grad_DB_name, $grand);

//Get graduands fee transaction payhistory 
$query_grand_fee = sprintf("SELECT a.* "
        . "FROM gradfees_transactions a "
        . "WHERE can_no = %s "
        . "OR can_no = %s "
        . "AND a.status = 'APPROVED' ", 
        GetSQLValueString($row_search['jambregid'], 'text'), 
        GetSQLValueString($row_search['stdid'], 'text'));
$Rs_grand_fee = mysql_query($query_grand_fee, $grand) or die(mysql_error());
$row_grand_fee = mysql_fetch_assoc($Rs_grand_fee);
$num_row_grand_fee = mysql_num_rows($Rs_grand_fee);
?>
<!doctype html>
                                                    ...
                                                    <tbody>
                                                        <?php if ($num_row_grand_fee > 0) { ?>
                                                            ...
                                                            do {
                                                                ...
                                                                <tr>
                                                                    <td><?= $i++; ?></td>
                                                                    <td><?= $row_grand_fee['can_no'] ?></td>
                                                                    <td><?= $row_grand_fee['reference'] ?></td>
                                                                    <td><?= $row_grand_fee['amt'] ?></td>
                                                                    <td><?= $row_grand_fee['date_time'] ?></td>
                                                                </tr>
                                                            <?php } while ($row_grand_fee = mysql_fetch_assoc($Rs_grand_fee)) ?>
                                                        <?php } else { ?>
                                                            <tr>
                                                                <td colspan="7"><div class="alert alert-warning"> No Payment Record</div></td>
                                                            </tr>
