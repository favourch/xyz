    ...
    if (is_uploaded_file($_FILES['filename']['tmp_name']) && in_array($_FILES['filename']['type'], $allowed_type)) {
        ...
        if(!empty($missing_entries)) {
            ...
            if(!empty($reg_entries)) {
                ...
                $query_reg = sprintf("INSERT INTO `course_reg` (stdid, sesid, csid, approved) VALUES %s;", 
        ...
        if (!empty($missing_entries)) {
            ...
            $uploadstat = "Unfortunately, the result file contained ". count($missing_entries)." unregistered students! "
                    . " Their results could not be uploaded <a target='_blank' href='http://my.tasued.edu.ng/admin/result/result_error.php?sid={$sesid}&csid={$csid}'>"
        ...
        $insert_query = sprintf("UPDATE teaching SET upload=%s WHERE csid=%s AND sesid=%s", 
...
if (isset($_POST['purge'])) {     
    ...
    $query_check = sprintf("SELECT * FROM teaching WHERE csid = %s AND sesid = %s", 
                    GetSQLValueString($csid, 'text'), 
                    GetSQLValueString($sesid, 'int'));
...
$query_crs = sprintf("SELECT t.csid "
                    . "FROM teaching t "
                    . "JOIN course c ON t.csid = c.csid "
                    . "WHERE upload != 'yes' "
                    . "AND sesid = %s "
                    . "AND c.catid IN (4,5,8) "
                    . "%s "
                    . "ORDER BY csid ASC", 
...
//Query for select boxes in the result view	
$query_ses = sprintf("SELECT s.sesname, s.sesid "
                    . "FROM session s "
...
$query_sescrs = sprintf("SELECT t.csid "
                        . "FROM teaching t JOIN course c ON t.csid = c.csid "
                        . "WHERE t.sesid=%s "
                        . "AND c.catid IN (4,5,8) "
                        . "AND upload = 'yes' "
...
$query_rslt = sprintf("SELECT r.resultid, r.csid, r.edited, r.stdid, r.sesid, tscore, escore, s.fname, s.lname "
                    . "FROM result r, student s, programme p, teaching t, course c "
                    ...
                    <div class="row-fluid">                        
                        <div class="span12">
                            ...
                            <div class="box box-bordered box-color" class="ng-cloak">
                                ...
                                <div class="box-content">
                                    <div class="row-fluid">
                                        <form class="form-vertical" method="POST" action="printresult.php" target="_blank">   
