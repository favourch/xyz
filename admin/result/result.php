...
if( isset($_POST) ){ //database query for approve result
	...
	if(isset($_POST['approve']) ){ //database query for approve result
        ...
        $deleteSQL11 = sprintf("DELETE FROM course_reg "
                                . "WHERE csid = %s "
                                . "AND sesid = %s", 
                                GetSQLValueString($csid, 'text'), 
                                GetSQLValueString($sesid, 'int'));
        $Result11 = mysql_query($deleteSQL11, $tams) or die(mysql_error());
		
		$updateSQL = sprintf("UPDATE teaching SET upload=%s, approve=%s, accepted=%s, released=%s WHERE deptid = %s AND csid = %s AND sesid = %s",
...
if (isset($_POST['newentries'])) { 
    ...
    if($totalRows_check > 0) {
        if((getAccess() == 2 && $row_check['approve'] == 'no') || 
                (getAccess() == 3 && $row_check['accepted'] == 'no')) {
...
$query_dept = sprintf("SELECT deptid, deptname FROM department ORDER BY deptid");
$dept = mysql_query($query_dept, $tams) or die(mysql_error());
$row_dept = mysql_fetch_assoc($dept);
$totalRows_dept = mysql_num_rows($dept);

$colname2_rslt = $row_dept['deptid'];

if (isset($_GET['did'])) {
	$colname2_rslt = $_GET['did'];
}

$query_status = sprintf("SELECT colid, approve, upload, accepted, released, type, l.fname, l.lname "
                        . "FROM course c, teaching t, department d, lecturer l "
                        . "WHERE c.deptid = d.deptid "
                        . "AND c.csid = t.csid "
                        . "AND t.lectid1 = l.lectid "
                        . "AND c.deptid = t.deptid "
...
$query_rslt = sprintf("SELECT r.resultid, r.edited, r.csid, r.stdid, tscore, escore, s.fname, s.lname, s.sex "
...
// find the session name for the result being considered - to be displayed
 $query_session = sprintf("SELECT * FROM session s "
                    . "WHERE sesid = %s ", 
                    GetSQLValueString($sesid, 'int'));
...
<html ng-app="TamsApp">
    ...
    <body data-layout-sidebar="fixed" data-layout-topbar="fixed" ng-controller="PageCtrl" class="ngcloak" ng-cloak>
                            ...
                            <div class="box box-bordered box-color">
                                 <div class="box-title">
                                    <h3><i class="icon-reorder"></i>
                                      <?php echo $row_session['sesname']; ?>:  Consider Result for <?php echo $name; ?>
                                    </h3>
                                </div>
                                ...
                                <div class="box-content"> 
                                    ...
                                    <div>
                                        ...
                                        <?php if ((getAccess() == 3 && !$accepted && !$approved) || (getAccess() == 2 && !$released)) { ?>
                                        ...
                                        <?php }?>
                                        <a href="statistics/index.php?crs=<?= $colname_rslt?>&sid=<?= $colname1_rslt?>" target="Tabs" class="btn btn-primary btn-small" style="margin-left: 20px">View Result Statistics</a>
                                    </div>
                                    ...
                                    <table class="table table-striped table-condensed table-hover">
                                        ...
                                        <tbody>
                                            <tr ng-repeat="rs in results" ng-if="!rs.deleted">
                                                ...
                                                <td align="center">
                                                    <a ng-if="rs.edited == 'TRUE'" target='_blank' href='../../result/edithistory.php?stdid={{rs.stdid}}&csid={{rs.csid}}&sid={{rs.sesid}}'>Edited</a>
                                                </td>
                                                <td align="center">
