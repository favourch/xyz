...
$query_RsdeptStd = sprintf("SELECT count(r.stdid) as `count`, d.deptname, d.deptid, d.colid "
...
// population by sex (M)
 $query_MaleStd = sprintf("SELECT distinct(count(s.stdid)) as `count`, d.deptname, d.colid "
                        . "FROM student s, department d, programme p, registration r "
                        . "WHERE r.progid = p.progid "
                        . "AND d.deptid = p.deptid "
                        . "AND r.stdid = s.stdid "
                        ...
                        . "ORDER BY d.deptid ASC",
                        GetSQLValueString($ses, "int")); 
$MaleStd = mysql_query($query_MaleStd) or die(mysql_error());
$row_MaleStd = mysql_fetch_assoc($MaleStd);
$totalRows_MaleStd = mysql_num_rows($MaleStd);


// population by sex (F)
$query_FemaleStd = sprintf("SELECT distinct(count(s.stdid)) as `count`, d.deptname, d.colid "
                        . "FROM student s, department d, programme p, registration r "
                        . "WHERE r.progid = p.progid "
                        . "AND d.deptid = p.deptid "
                        . "AND r.stdid = s.stdid "
                        ...
                        . "ORDER BY d.deptid ASC",
                        GetSQLValueString($ses, "int"));
$FemaleStd = mysql_query($query_FemaleStd) or die(mysql_error());
$row_FemaleStd = mysql_fetch_assoc($FemaleStd);
$totalRows_FemaleStd = mysql_num_rows($FemaleStd);
...
$query_MaleColStd = sprintf("SELECT distinct(count(s.stdid)) as `count`, c.colname "
                        . "FROM student s, department d, programme p, college c, registration r "
                        . "WHERE s.progid = p.progid "
                        . "AND d.deptid = p.deptid "
                        . "AND r.stdid = s.stdid "
                        ...
                        . "ORDER BY c.colid ASC",
                        GetSQLValueString($ses, "int"));
$MaleColStd = mysql_query($query_MaleColStd) or die(mysql_error());
$row_MaleColStd = mysql_fetch_assoc($MaleColStd);
$totalRows_MaleColStd = mysql_num_rows($MaleColStd);

$query_FemaleColStd = sprintf("SELECT distinct(count(s.stdid)) as `count`, c.colname "
                        . "FROM student s, department d, programme p, college c, registration r "
                        . "WHERE s.progid = p.progid "
                        . "AND d.deptid = p.deptid "
                        . "AND r.stdid = s.stdid "
                        ...
                        . "ORDER BY c.colid ASC",
                        GetSQLValueString($ses, "int"));
$FemaleColStd = mysql_query($query_FemaleColStd) or die(mysql_error());
$row_FemaleColStd = mysql_fetch_assoc($FemaleColStd);
$totalRows_FemaleColStd = mysql_num_rows($FemaleColStd);
...
//  $query_accstud = sprintf("SELECT count(distinct(at.can_no)) AS `count`, d.deptid, c.colid, entrylevel, sex "
...
//                            . "GROUP BY d.deptid, c.colid, a.typeid, sex "
...
//        
//        // Sex segment
//        if(isset($additions[$row_accstud['colid']][$row_accstud['deptid']][$row_accstud['sex']])){
//            $additions[$row_accstud['colid']][$row_accstud['deptid']][$row_accstud['sex']] += $row_accstud['count'];
//        }else {
//            $additions[$row_accstud['colid']][$row_accstud['deptid']][$row_accstud['sex']] = $row_accstud['count'];
//        }
//        
//        if(isset($additions[$row_accstud['colid']][$row_accstud['sex']])){
//            $additions[$row_accstud['colid']][$row_accstud['sex']] += $row_accstud['count'];
//        }else {
//            $additions[$row_accstud['colid']][$row_accstud['sex']] = $row_accstud['count'];
//        }
//        
...
<html>
                                    ...
                                    <div class="row-fluid">
                                        <table class="table table-condensed table-striped" >
                                            ...
                                            do {
                                                ...
                                                $totalMale = $totalMale + $row_MaleStd['count'];
                                                $totalFemale = $totalFemale + $row_FemaleStd['count'];                                                
 ...
 <td>
                                                        <a target="_blank" href="studentlist.php?action=pmal&did=<?php echo $row_Rsdept['deptid'] ?>&sid=<?php echo $ses ?>">
                                                            <?php echo $row_MaleStd['count'] ?>
                                                        </a>
                                                    </td>

 <td>
                                                        <a target="_blank" href="studentlist.php?action=pfem&did=<?php echo $row_Rsdept['deptid'] ?>&sid=<?php echo $ses ?>">
                                                            <?php echo $row_FemaleStd['count'] ?>
                                                        </a>
                                                    </td>
                                            ...
                                            <tr>
                                                ...
                                                <th align="center"><?php echo $totalMale ?></th>
                                                <th align="center"><?php echo $totalFemale ?></th>
                                            ...
                                            <table class="table table-condensed table-striped" >
                                                ...
                                                do {
                                                    ...
                                                    <tr align="center" >
                                                        ...
                                                        <td>
                                                            <a target="_blank" href="studentlist.php?action=paid&cid=<?php echo $row_Rscol['colid'] ?>&sid=<?php echo $ses ?>&lvl=1">
                                                                <?php echo isset($statCPaid[$row_Rscol['colid']][1]) ? $statCPaid[$row_Rscol['colid']][1] : 0; ?>
                                                            </a> | 
                                                            <a target="_blank" href="studentlist.php?cid=<?php echo $row_Rscol['colid'] ?>&sid=<?php echo $ses ?>&lvl=1">
                                                                <?php echo $statCPop[$row_Rscol['colid']][1] ?>
                                                            </a>
                                                        </td>

                                                        <td>
                                                            <a  target="_blank"href="studentlist.php?action=paid&cid=<?php echo $row_Rscol['colid'] ?>&sid=<?php echo $ses ?>&lvl=2">
                                                                <?php echo $statCPaid[$row_Rscol['colid']][2] ?>
                                                            </a> | 
                                                            <a target="_blank" href="studentlist.php?cid=<?php echo $row_Rscol['colid'] ?>&sid=<?php echo $ses ?>&lvl=2">
                                                                <?php echo isset($statCPop[$row_Rscol['colid']][2]) ? $statCPop[$row_Rscol['colid']][2] : 0; ?>
                                                            </a>
                                                        </td>

                                                        <td>
                                                            <a target="_blank" href="studentlist.php?action=paid&cid=<?php echo $row_Rscol['colid'] ?>&sid=<?php echo $ses ?>&lvl=3">
                                                                <?php echo $statCPaid[$row_Rscol['colid']][3] ?>
                                                            </a> | 
                                                            <a target="_blank" href="studentlist.php?cid=<?php echo $row_Rscol['colid'] ?>&sid=<?php echo $ses ?>&lvl=3">
                                                                <?php echo isset($statCPop[$row_Rscol['colid']][3]) ? $statCPop[$row_Rscol['colid']][3] : 0 ?>
                                                            </a>
                                                        </td>

                                                        <td>
                                                            <a target="_blank" href="studentlist.php?action=paid&cid=<?php echo $row_Rscol['colid'] ?>&sid=<?php echo $ses ?>&lvl=4">
                                                                <?php echo $statCPaid[$row_Rscol['colid']][4] ?>
                                                            </a> | 
                                                            <a target="_blank" href="studentlist.php?cid=<?php echo $row_Rscol['colid'] ?>&sid=<?php echo $ses ?>&lvl=4">
                                    ...
                                    <div class="row-fluid">
                                        ...
                                        <table class="table table-condensed table-striped" >
                                            ...
                                            do {
                                                ...
                                                $totalColMale = $totalColMale + $row_MaleColStd['count'];
                                                $totalColFemale = $totalColFemale + $row_FemaleColStd['count'];
...
<td>
                                                        <a target="_blank" href="studentlist.php?action=colm&cid=<?php echo $row_Rscol['colid'] ?>&sid=<?php echo $ses ?>">
                                                            <?php echo $row_MaleColStd['count'] ?>
                                                        </a>
                                                    </td>

<td>
                                                        <a target="_blank" href="studentlist.php?action=colf&cid=<?php echo $row_Rscol['colid'] ?>&sid=<?php echo $ses ?>">
                                                            <?php echo $row_FemaleColStd['count'] ?>
                                                        </a>
                                                    </td>
                                                    ...
                                                    <td>
                                                        <?php 
                                                            if (!isset($colpaid[$row_Rscol['colid']]))
                                                                    $colpaid[$row_Rscol['colid']] = 0;
                                                                echo round($colpaid[$row_Rscol['colid']] / $row_RscolStd['count'] * 100, 2) ?>
                                                    </td>
                                                </tr>
                                            ...
                                            <tr>
                                                ...
                                                <th align="center"><?php echo $totalColMale ?></th>
                                                <th align="center"><?php echo $totalColFemale ?></th>
