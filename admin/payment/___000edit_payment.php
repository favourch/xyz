...
if (isset($_GET['id']) && $_GET['id'] != '' && isset($_GET['type']) && $_GET['type'] != '') {
    ...
    switch ($type) {
        ...
        case 'pros':
            ...
            $query_stdinfo = sprintf("SELECT jambregid as stdid, fname, lname, sesid "
                                    . "FROM prospective "
                                    . "WHERE jambregid = %s", 
                                    GetSQLValueString($stdid, "text"));
            ...
            if (!empty($update_array)) {
                ...
                if ($Result) {
                    ...
                    if($cat == 'olevelfee'){
                        ...
                        if($ver_num_rows < 1){
                            $sitting = 1;
                            if(explode('.',substr($row_trans['amt'], 3))[0] > 500){
                               $sitting = 2 ;
                            }
                            
                           
                            $verificationSQL = sprintf("INSERT INTO verification "
                                                     . "(stdid, sesid, type, ver_code, ordid, olevel_sitting ) "
