...
//$verify_row_num = mysql_num_rows($verify);
    ...
    if(isset($_SESSION['olv_veri_type']) && $_SESSION['olv_veri_type'] == "DE"){
        ...
        $query = sprintf("SELECT v.*, p.*,c.colid, c.colname, d.deptid, d.deptname,at.typename, prg.progname, p.jambregid as stdid, rt.regtypename, rt.regtypeid "
                . "FROM verification v "
          ...
          $query = sprintf("SELECT v.*, p.*,su1.subjname AS subj1, su2.subjname AS subj2,su3.subjname AS subj3, su4.subjname AS subj4,c.colid, c.colname, d.deptid, d.deptname, at.typename, prg.progname, p.jambregid as stdid, rt.regtypename, rt.regtypeid "
                . "FROM verification v "
                . "JOIN prospective p ON v.jambregid = p.jambregid "
                . "JOIN admissions a ON a.admid = p.admid "
...
while ($verify_row = mysql_fetch_assoc($verify)) {
   ...
   $code[] = array(
            ...
            'score' =>  (int)$verify_row['score'],
            'jamb_total' =>  (int)$verify_row['jamb_total']
        );  
}
...
if(isset($_POST['release'])){
    ...
    $msg = "Congratulations! <br /> Your O`level result has been"
            . " reviewed and it meets the minimum admission requirement of the"
...
if(isset($_POST['MM_Submit']) && $_POST['MM_Submit'] == 'change_prog'){
    ...
    if ($_POST['action'] == 'yes') {
        ...
        $msg = " Your O`level result has been reviewed; "
                . "it does not meet the minimum admission requirement for the programme that you applied for "
                . " (<strong>{$_POST['progname']}</strong>).<br/><br />"
                . " However, you have been considered for another programme. "
                . "You have, therefore, been offered provisional admission "
                . "to pursue a Full-Time Bachelor's Degree in <strong>{$list}</strong>. <br /><br />"
                . "Login to the portal - https://my.tasued.edu.ng to check your Admission Status & pay Acceptance Fee online ";
        ...
        $SQL2 = sprintf("UPDATE prospective "
                . "SET adminstatus = 'yes', progoffered = %s "
                . "WHERE jambregid = %s ",
                GetSQLValueString(implode(',', $new_prog), 'int'),
                GetSQLValueString($cur_user, 'text'));
        mysql_query($SQL2, $tams) or die(mysql_error());
...
if(isset($_POST['terminate'])){
    ...
    $msg = "Sorry, your O'Level Result does not meet the minimum admission requirement for the "
            ...
            . "Your application for an Offer of Provisional Admission is denied. However, you can contact the Admissions Office for further advice";
            ...
            <form method="post" action="process.php">
                ...
                <div class="modal-footer">
                    ...
                    <a class="btn btn-warning" 
                        ...
                        data-toggle="modal"data-dismiss="modal" aria-hidden="true" ng-click="setSelected(seletedItem)">Change Program</a>
            ...
            <form method="post" action="process.php">
                <div class="modal-body">
                    ...
                    <div class="row-fluid" ng-if="fetched_result.olevel" >
                        ...
                        <div class="span6 well well-large" ng-repeat="result in fetched_result.olevel" ng-bind-html="renderHTML(result.result_table)"></div>
                    </div>
                </div>
                ...
                <div class="modal-footer">
                    ...
                    <?php if(in_array(getAccess(), ['20','24'])){?>
                        ...
                        <a class="btn btn-warning" 
                            href="#change_prog" role="button" 
                            data-toggle="modal"data-dismiss="modal" aria-hidden="true" ng-click="setSelected(seletedItem)">Change Programme
                        </a>
                        <button type="submit" name="terminate" class="btn btn-danger">Deny Admission</button>
                    <?php }?>
                    <button class="btn" data-dismiss="modal" aria-hidden="true">Close</button>
